...
/**
 * Request albums information from the Spotify APIs.
 */
class AlbumService extends ResourceService {
  ...
  public function save(array $ids): void {
    $response = $this->spotifyRequest->getResource('albums', $ids);
    $response = json_decode($response->getBody()->getContents(), TRUE);

    foreach ($response['albums'] as $album) {

      // Get the artist resource because the song endpoint don't return images.
      $artistEntity = $this->artistRequest->saveSingleArtist($album['artists'][0]['id']);
      if ($artistEntity) {
        $albumModel = new Album($album, $artistEntity);
        $albumEntity = $this->getEntity($albumModel);
        $albumEntity = $albumModel->save($albumEntity);

        foreach ($album['tracks']['items'] as $track) {
          $song = new Song($track, $artistEntity, $albumEntity);
          $song->setPicture($albumModel->picture);
